...
import copy
import time
# calculate ratio of items (value/weight)/distance_first_city, sort in ascending order
...
def order_objects_by_value_and_distance(cities, origin) :
    """
    Orders objects by their ratio value/weight ponderated by the distance from the origin
    returns a list of tuples (index, ratio, weight, cityKey)
    cities is a dictionary where an entry  is key (int) : [(pos_x, pos_y), [(object_index, profit, weight), ...]]
    """
    distances = distances_to_city(origin, cities)
    objects = []

    for cityKey, value in cities.items() :
        for o in value[1] :
            ratio = utils.calculate_value_ratio(o) / distances[cityKey]
            objects.append((o[0], ratio, o[2], cityKey)) # index, ratio, weight, cityKey
    
    objects.sort(key=utils.sortSecond, reverse=True)
    return objects

def select_items_linear(cities, capacity, origin) :
    """
    ...
    """
    objects = order_objects_by_value_and_distance(cities, origin)
    selected_objects = []
    fill = 0
...
def tour_with_knapsack_linear(cities, capacity):
    """
    Calculates the most interesting objects that are close to the origin
    constructs a tour that goes to the closest neighbor and finishes by the cities with objects
    cities is a dictionary where an entry  is key (int) : [(pos_x, pos_y), [(object_index, profit, weight), ...]]
    capacity (int) the maximum capacity of the knapsack
    """
    selected_objects = select_items_linear(cities, capacity, 1)
...
# with our idea
# t, objects = tour_with_knapsack_linear(cities, capacity)
# print("Value of objective function : ",utils.objective_function_linear(cities, objects, renting_ratio, t))#, max_speed, min_speed, capacity))
